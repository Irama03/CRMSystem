...
public class TaskServiceImpl implements TaskService {
    ...
    @Autowired
    ExternalRequestsServiceImpl externalRequestsService;
    ...
    public Iterable<Task> getAllAllowedTasks(Long workerId) {
        Worker worker = externalRequestsService.getWorkerById(workerId);
        Worker head = externalRequestsService.getHeadOfDepartment(worker.getDepartmentId());
        return taskRepository.getAllAllowedTasks(workerId, head.getId());
    }
    ...
    public Task updateTask(Task task, Set<Task> previousTasks) {
        ...
        if (t.getState() != StateType.NEW) {
            ...
            if (!t.isTemplate()) {
                ...
                t.setResponsibleWorkerId(task.getResponsibleWorkerId());
                t.setPerformerId(task.getPerformerId());
            }
    ...
    private void processNotTemplateCommonFields(Task task, Task source, Set<Task> previousTasks) {
        ...
        //responsibleWorker
        if (source.getResponsibleWorkerId() == null)
            throw new TaskIllegalArgumentException(Values.TASK_NEEDS_RESPONSIBLE_WORKER);
        Worker responsible = externalRequestsService.getWorkerById(source.getResponsibleWorkerId());
        if (!responsible.isHead())
            throw new TaskIllegalArgumentException(Values.ONLY_HEAD_CAN_BE_RESPONSIBLE_FOR_TASK);
        //performer
        if (source.getPerformerId() != null) {
            Worker performer = externalRequestsService.getWorkerById(source.getPerformerId());
            if (!performer.getDepartmentId().equals(responsible.getDepartmentId()))
                throw new TaskIllegalArgumentException(Values.PERFORMER_SHOULD_BE_IN_DEPARTMENT_OF_RESPONSIBLE_WORKER);
        }
        else task.setPerformerId(responsible.getId());
        //previousTasks
        if (previousTasks == null || previousTasks.isEmpty()) {
